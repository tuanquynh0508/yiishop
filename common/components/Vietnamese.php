...
class Vietnamese {
	...
	// PRIVATE 
	private function dochangchuc($so, $daydu) {
		$chuoi = "";
		$chuc = floor($so / 10);
		$donvi = $so % 10;
		if ($chuc > 1) {
	...
	private function docblock($so, $daydu) {
		$chuoi = "";
		$tram = floor($so / 100);
		$so = $so % 100;
		if ($daydu || $tram > 0) {
	...
	private function dochangtrieu($so, $daydu) {
		$chuoi = "";
